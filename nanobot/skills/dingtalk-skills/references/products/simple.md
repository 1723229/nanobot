...
- 如果处理本文覆盖的产品请求时遇到 `AUTH_TOKEN_EXPIRED` / `USER_TOKEN_ILLEGAL` / "Token验证失败"，先由 agent 在后台通过 `python <skill_dir>/scripts/auth_device_session.py status --session "<SENDER_ID>"` 统一处理认证；如果返回授权链接，用户可见回复只返回授权链接和“完成授权后我继续处理当前请求”的提示，不要让用户执行命令。
