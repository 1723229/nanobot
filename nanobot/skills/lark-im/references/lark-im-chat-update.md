...
| Permission denied (99991679) | Missing `im:chat:update` permission | Follow the non-blocking auth rule in [`../../lark-shared/SKILL.md`](../../lark-shared/SKILL.md): have the agent start `auth login --no-wait --scope "im:chat:update"` internally and return only the authorization link; do not ask the end user to run the command manually |
