...
| Permission denied (99991679) | Missing `im:chat:update` permission | Follow the shared auth rule in [`../../lark-shared/SKILL.md`](../../lark-shared/SKILL.md): run `python <skill_dir>/scripts/auth_link.py login --scope "im:chat:update" --timeout 30` internally, extract `auth_url`, and return only that link to the user |
