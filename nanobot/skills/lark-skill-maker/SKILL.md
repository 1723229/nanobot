...
- **认证** — 说明所需 scope，并明确要求 agent 按共享规则在后台发起非阻塞授权命令（例如 `lark-cli auth login --no-wait --domain <name>` 或 `lark-cli auth login --no-wait --scope "<scope>"`）、仅向用户返回授权链接，不要让最终用户执行命令
