...
> - `status = skipped`：本地没有可用的当前用户 `open_id`，或当前结果还没有可授权目标，因此不会自动授权；如需补齐用户授权，按 [`../../lark-shared/SKILL.md`](../../lark-shared/SKILL.md) 的非阻塞认证规则处理，并把授权链接发给用户点击，再继续使用应用身份（bot）授予当前用户权限
