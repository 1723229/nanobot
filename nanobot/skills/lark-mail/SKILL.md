...
- **`--as user`（推荐）**：以当前登录用户的身份访问其邮箱。如用户授权缺失或过期，按 [`../lark-shared/SKILL.md`](../lark-shared/SKILL.md) 的非阻塞认证规则在后台发起授权，并只把授权链接发给用户点击；不要让最终用户执行命令。
- **`--as bot`**：以应用身份访问邮箱。需要在飞书开发者后台为应用开通相应权限，否则请求会被拒绝。**注意：bot 身份仅适用于读取类操作，所有写操作（发送、回复、转发、草稿编辑等）仅支持 user 身份。**

1. 所有邮件写操作（发送、回复、转发、草稿编辑） → 必须使用 `--as user`；如用户授权缺失或过期，按 [`../lark-shared/SKILL.md`](../lark-shared/SKILL.md) 的非阻塞认证规则处理并返回授权链接
