...
```bash
lark-cli auth login --no-wait --domain <domain>           # 按业务域授权
lark-cli auth login --no-wait --scope "<missing_scope>"   # 按具体 scope 授权（推荐,符合最小权限原则）
```

**规则**：auth login 必须指定范围（`--domain` 或 `--scope`）。多次 login 的 scope 会累积（增量授权）。优先使用 `--no-wait` 发起非阻塞认证。
...
```bash
# 内部发起授权（非阻塞）
lark-cli auth login --no-wait --scope "calendar:calendar:readonly"
```
